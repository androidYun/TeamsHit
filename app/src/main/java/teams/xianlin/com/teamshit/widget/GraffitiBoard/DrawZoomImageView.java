package teams.xianlin.com.teamshit.widget.GraffitiBoard;
...
public class DrawZoomImageView extends View {
    ...
    public void revoke() {
        ...
        LogUtil.d(TAG, "撤销前有" + mPathList.size() + "条线");
    ...
    public void recovery() {
        if (currentStatus == STATUS_INIT)//还未植入图片
            return;
        LogUtil.d(TAG, "恢复前有" + mPathList.size() + "条线");
        mCanvas.drawColor(Color.TRANSPARENT, Mode.CLEAR);    //清空画布
        //画线
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                if (event.getPointerCount() == 1) {
                    ...
                    mDp = new DrawPath();
                    mDp.path = mPath;
                    LogUtil.d(TAG, "手指放下，当前线术" + mPathList.size());
            ...
            case MotionEvent.ACTION_POINTER_UP:  //当第二根手指抬起
                if (event.getPointerCount() == 1) {
                    ...
                    float y = event.getY();
                    if (hasLine) {
                        LogUtil.d(TAG, "保存有效线条");
                        LogUtil.d(TAG, "手指抬起了，将线条放入集合，数量" + mPathList.size());
    ...
    private void initBitmap(Canvas canvas) {
        if (sourceBitmap != null) {
            ...
            LogUtil.d(TAG, "图片的宽高" + bitmapWidth + ", " + bitmapHeight + "，控件" + width + "," + height);
            if (bitmapWidth > width || bitmapHeight > height) {
                if (bitmapWidth - width > bitmapHeight - height) {
                    LogUtil.d(TAG, bitmapWidth + "图片的宽度太大了" + width);
                    // 当图片宽度大于屏幕宽度时，将图片等比例压缩，使它可以完全显示出来
                    float ratio = width / (bitmapWidth * 1.0f);
                    LogUtil.d(TAG, bitmapWidth + "图片的宽度太大了" + width + "，按照宽度缩放" + ratio);
                    ...
                    LogUtil.d(TAG, bitmapWidth + "图片的高度太大了" + width + "，按照高度缩放" + ratio);
                    matrix.postScale(ratio, ratio);
                ...
                LogUtil.d(TAG, "图片的宽高都小于屏幕宽高" + "，X平移" + translateX + ",Y平移" + translateY);
                matrix.postTranslate(translateX, translateY);
                totalTranslateX = translateX;
                totalTranslateY = translateY;
            ...
            LogUtil.d(TAG, "压缩后图片宽高" + currentBitmapWidth + " " + currentBitmapHeight);
            //创建一副原图大小相同的透明背景图片
...
//			bgBitmap = Bitmap.createBitmap(sourceBitmap,0,0,sourceBitmap.getWidth(),sourceBitmap.getHeight());
            ...
            LogUtil.d(TAG, "创建一个宽高为" + mCanvas.getWidth() + " " + mCanvas.getHeight() + "的画布");
