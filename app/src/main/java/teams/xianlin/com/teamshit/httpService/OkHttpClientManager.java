...
import teams.xianlin.com.teamshit.NetWorkRequest.PacketRequest;
...
import teams.xianlin.com.teamshit.Utils.JsonUtil;
...
public class OkHttpClientManager {
    MediaType JSON = MediaType.parse("application/json; charset=utf-8");//设置请求编码格式
    private static final MediaType MEDIA_TYPE_PNG = MediaType.parse("image/png");//上传多张图片
    ...
    private void _postAsyn(String url, final ResultCallback callback, Param... params) {
        Request request = buildPostRequest(url, params);
        LogUtil.d("" + request.method().toString());
        deliveryResult(callback, request);
    }
     ...
     * @param packet
     */
    private void _postAsyn(String url, final ResultCallback callback, PacketRequest packet) {
        String toJson = JsonUtil.toJson(packet);
        LogUtil.d("DSFSD" + toJson);
        Request request = buildPostRequest(url, toJson);
        deliveryResult(callback, request);
    }
    ...
    private void _postAsyn(String url, ResultCallback callback, File file, String fileKey) throws IOException {
        LogUtil.d("Url" + url + "    " + file.getAbsolutePath());
        Request request = buildMultipartFormRequest(url, new File[]{file}, new String[]{fileKey}, null);
        deliveryResult(callback, request);
    }
    ...
    public void getAsyn(String url, ResultCallback callback, int respCode) {
    ...
    public void postAsyn(String url, final ResultCallback callback, int respCode, Param... params) {

        this.respCode = respCode;
        getInstance()._postAsyn(url, callback, params);
    }

    public void postAsyn(String url, final ResultCallback callback, int respCode, PacketRequest packet) {
        this.respCode = respCode;
        getInstance()._postAsyn(url, callback, packet);
    }
    ...
    private Request buildMultipartFormRequest(String url, File[] files,
        ...
        for (Param param : params) {
            builder.addPart(Headers.of("Content-Disposition", "form-data; name=\"" + param.key + "\""),
                    RequestBody.create(MEDIA_TYPE_PNG, String.valueOf(param.value)));
        }
        if (files != null) {
        ...
        mOkHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(final Response response) {
                try {
                    ...
                    LogUtil.d(respCode + "响应数据成功" + string);
    ...
    private Request buildPostRequest(String url, Param[] params) {
        ...
        if (isTest) {
            LogUtil.d("请求接口" + respCode % 10 + "url" + url + "请求数据" + requestData + "}");
        } else {
            LogUtil.d("请求接口" + respCode % 10 + "请求数据" + requestData + "}");
        }
        RequestBody requestBody = builder.build();
        return new Request.Builder()
                .url(url)
